...
const handleSearch = async () => {
    try {
        ...
        if (idBus) searchValue = `BUS-${idBus}`;
        else if (idLivraison) searchValue = `LIV-${idLivraison}`;
    ...
    const response = await axios.get(
      `http://localhost:8080/location-params/${searchValue}`,
      {
        headers: {
          Authorization: `Bearer ${bearerToken}` // <-- add the token here
        }
      }
    );
        setLastReferencedLocation(response.data.city);
